        ...
        it('should convert a document and preserve styles', () => {
            return ArchieConverter.convert(html, {preserve_styles: ['bold', 'italic', 'underline']})
                .then(({result}) => {
                    ...
                    assert.equal(result.body[1].value, '<span class="g-doc-bold">line 2 bold with non-bold space</span>');
                    assert.equal(result.body[2].value, 'line 3 entire line bold ignored');
                    assert.equal(result.body[3].value, 'line 4 with <span class="g-doc-underline">partial underline</span>');
                })
        });
        ...
        it('should convert a document with no styles', () => {
            return ArchieConverter.convert(html)
                .then(({result}) => {
                    ...
                    assert.equal(result.body[0].value, 'line 1 not bold');
                    assert.equal(result.body[1].value, 'line 2 bold with non-bold space');
                    assert.equal(result.body[2].value, 'line 3 entire line bold ignored');
